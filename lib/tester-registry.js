...
export default class TesterRegistry {
  emitter: Emitter;
  testers: Set<Tester>;
  locks: Set<EditorTester|ResultView>;
  ...
  async test(editorTester : EditorTester|ResultView, shouldTestCurrentProject :?boolean) : Promise<boolean> {
    console.log('tester-registry', shouldTestCurrentProject);
    let editor;
    if (editorTester instanceof ResultView) {
      editor = editorTester;
    } else {
    ...
    this.testers.forEach((tester) => {
      ...
      .then(({ messages, output }) => {
        if (messages) {
          this.emitter.emit('did-update-messages', { tester, editor, messages, output, shouldTestCurrentProject });
        }
      })
      .catch((error) => {
        console.error('Tester: ', error);
