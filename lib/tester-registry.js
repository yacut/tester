...
export default class TesterRegistry {
  ...
  async test(editor/* : TextEditor*/)/* : Promise<boolean> */ {
    if (!editor.getPath() || this.locks.has(editor)) {
      return Promise.resolve(true);
    }

    this.emitter.emit('did-begin-testing', { editor });
    this.locks.add(editor);
    ...
    await Promise.all(promises)
      .then(() => {
        this.locks.delete(editor);
        this.emitter.emit('did-finish-testing', { editor });
      })
      .catch((error) => {
        atom.notifications.addError(`Tester error: ${error.toString()}`);
        return false;
