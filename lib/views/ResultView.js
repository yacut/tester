...
export default class ResultView {
  ...
  serialize() {
    return {
      deserializer: 'tester-result-view/ResultView',
    };
  }
  ...
  goToNextTest() {
    ...
    if (!this.currentTest) {
      this.currentTest = this.properties.messages[0];
    } else {
      const index = this.properties.messages.indexOf(this.currentTest);
      if (index >= 0 && index < this.properties.messages.length - 1) {
        this.currentTest = this.properties.messages[index + 1];
      }
    }
  ...
  goToPreviousTest() {
    ...
    if (!this.currentTest) {
      this.currentTest = this.properties.messages[this.properties.messages.length - 1];
    } else {
      const index = this.properties.messages.indexOf(this.currentTest);
      if (index > 0 && index <= this.properties.messages.length - 1) {
        this.currentTest = this.properties.messages[index - 1];
  ...
  handleRowClick(selectedIndex: number): void {
    if (!this.properties.messages || this.properties.messages.constructor !== Array) {
      return;
    }
