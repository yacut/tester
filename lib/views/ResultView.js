...
import type { Message, TesterState } from '../types';
...
export default class ResultView {
  ...
  constructor(properties:Object) {
    ...
    const currentFileOnlyHandler = async () => {
      this.properties.currentFileOnly = this.refs.currentFileOnly.checked;
      await this.update(this.properties.state, true);
    };
    this.refs.softWrap.addEventListener('click', currentFileOnlyHandler);
    this.disposables.add(new Disposable(() => { this.refs.currentFileOnly.removeEventListener('click', currentFileOnlyHandler()); }));
  }
        ...
        <div class='tester-toolbar inline-block'>
          ...
          <div class='inline-block pull-right'>
            <label class='input-label inline-block-tight'>
              <input class='input-checkbox' ref='currentFile' type='checkbox' checked={this.properties.currentFileOnly}/> Current file only</label>
            <label class='input-label inline-block-tight'>
  ...
  update(newState :TesterState, shouldUpdate :?boolean) {
    if (newState && this.properties.state !== newState) {
      ...
      if (this.properties.currentFileOnly && this.properties.state.editor) {
        const currentFilePath = this.properties.state.editor.getPath();
        this.properties.state.messages = this.properties.state.messages.filter(message => message.filePath === currentFilePath);
      }
  ...
  handleTestButtonClick(): ResultView {
    this.emitter.emit('test-project-button-click', this.getAdditionalArgs());
    return this;
  }

  handleRowClick(selectedIndex: number): ResultView {
  ...
  handleSortByClick(key: string): ResultView {
