    ...
    return (
      <div class='tester-output-view'>
        ...
        <div class='tester-messages' ref='messages' messages={this.properties.messages}>
          <div class='tester-messager-head tester-message-row inline-block'>
            title
          </div>
          {this.properties.messages.map((message) => {
            console.log('message', message);
            return <div class='tester-message-row inline-block'>
                    <div class='tester-message-state inline-block'><span class='highlight-info'>{message.state || 'unknown'}</span></div>
                    <div class='tester-message-title inline-block'>{message.title || ''}</div>
                    <div class='tester-message-duration inline-block pull-right'>{message.duration || 0}</div>
                    <div class='tester-message-filepath inline-block pull-right'>{message.filePath || ''}</div>
                    <div class='tester-message-linenumber inline-block pull-right'>:{message.lineNumber || 0}</div>
              </div>;
          })}
        </div>
      </div>
