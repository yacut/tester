...
import type { TesterState, Column, ColumnKey, WidthMap } from '../types';
...
export default class ResultView {
  ...
  constructor(properties: {
    ...
    (this: any).handleResizerGlobalMouseUp = this.handleResizerGlobalMouseUp.bind(this);
    (this: any).handleResizerGlobalMouseMove = this.handleResizerGlobalMouseMove.bind(this);
    if (!this.properties.state.resultViewColumns ||
      !this.properties.state.resultViewColumns.columns ||
      !this.properties.state.resultViewColumns.columnWidthRatios) {
      const columns = [
          { key: 'state', width: 0.03 },
          { key: 'duration', width: 0.05 },
          { key: 'title', width: 0.3 },
          { key: 'error', width: 0.5 },
          { key: 'location', width: 0.12 },
      ];
      const columnWidthRatios = this.getInitialWidthsForColumns(columns);
      this.properties.state.resultViewColumns = {
        columns,
        columnWidthRatios,
      };
    }
  ...
  render() {
    ...
    const { columnWidthRatios } = this.properties.state.resultViewColumns || {};
        ...
        <div class='tester-messages' ref='messages' messages={messages}>
          <div class='tester-message-header inline-block'>
              <div class={
                ...
                style={`width: ${columnWidthRatios.state * 100}%`}>
              ...
              <div class='tester-message-cell tester-header tester-message-duration inline-block'
                ...
                style={`width: ${columnWidthRatios.duration * 100}%`}>Duration
              ...
              <div class='tester-message-cell tester-header tester-message-title inline-block'
                ...
                style={`width: ${columnWidthRatios.title * 100}%`}>Title
              ...
              <div class='tester-message-cell tester-header tester-message-error inline-block'
                ...
                style={`width: ${columnWidthRatios.error * 100}%`}>Error
              ...
              <div class='tester-message-cell tester-header tester-message-location inline-block'
                ...
                style={`width: ${columnWidthRatios.location * 100}%`}>Location
            ...
            <div class='tester-message-row inline-block'
              ...
              onclick={this.handleRowClick.bind(this, index)}>
                    ...
                    <div class='tester-message-cell tester-message-duration inline-block'
                      style={`width: ${columnWidthRatios.duration * 100}%`}>{message.duration || 0}ms</div>
                    <div class='tester-message-cell tester-message-title inline-block'
                      style={`width: ${columnWidthRatios.title * 100}%`}>{message.title || ''}</div>
                    <div class='tester-message-cell tester-message-error inline-block'
                      style={`width: ${columnWidthRatios.error * 100}%`}>
                    ...
                    <div class='tester-message-cell tester-message-location inline-block' ref='tester-message-location'
                      style={`width: ${columnWidthRatios.location * 100}%`}>
  ...
  onSetResultViewColumns(callback: Function) : Disposable {
    return this.emitter.on('set-result-view-columns', callback);
  }
  ...
  handleResizerGlobalMouseMove(event: any): void {
    ...
    const { columnWidthRatios, columns } = this.properties.state.resultViewColumns;
    const currentColumnSize = columnWidthRatios[this.columnBeingResized];
    const updatedColumnWidths = this.updateWidths(this.columnBeingResized, ((this.tableWidth * currentColumnSize) + deltaX) / this.tableWidth);
    if (updatedColumnWidths) {
      this.resizeStartX = pageX;
      this.emitter.emit('set-result-view-columns', { columns, columnWidthRatios: updatedColumnWidths });
    }
  }

  updateWidths(resizedColumn: string, newColumnSize: number): ?WidthMap {
    if (!this.properties.state.resultViewColumns) {
      return null;
    }
    const { columnWidthRatios, columns } = this.properties.state.resultViewColumns;
