...
export default class ResultView {
  ...
  constructor(properties:Object) {
    ...
    const softWrapHandler = async () => {
      await this.update({ softWrap: this.refs.softWrap.checked });
    };
        ...
        <div class='tester-messages' ref='messages' messages={this.properties.messages}>
          <div class='tester-message-header inline-block'>
              <div class='tester-message-cell tester-header tester-message-state inline-block'
                ref='header-state' onclick={this.handleSortByClick.bind(this, 'state')} >State</div>
              <div class='tester-message-cell tester-header tester-message-duration inline-block'
                ref='header-duration' onclick={this.handleSortByClick.bind(this, 'duration')}>Duration</div>
              <div class='tester-message-cell tester-header tester-message-title inline-block'
                ref='header-title' onclick={this.handleSortByClick.bind(this, 'title')}>Title</div>
              <div class='tester-message-cell tester-header tester-message-error inline-block'
                ref='header-error' onclick={this.handleSortByClick.bind(this, 'error')}>Error</div>
              <div class='tester-message-cell tester-header tester-message-location inline-block'
                ref='header-filePath' onclick={this.handleSortByClick.bind(this, 'filePath')}>Location</div>
          </div>
            ...
            <div class='tester-message-row inline-block'
              ...
              onclick={this.handleRowClick.bind(this, index)}>
                    ...
                    <div class='tester-message-cell tester-message-location inline-block' ref='tester-message-location'>
                      {atom.project.relativizePath(message.filePath)[1] || message.filePath || ''}:{message.lineNumber || ''}
                    </div>
              </div>)}
          </div>
        </div>
  ...
  async handleSortByClick(key: string) {
    ...
    this.properties.messages.sort((current, next) => {
      const currentValue = key === 'error' ? current.error.message : current[key];
      const nextValue = key === 'error' ? next.error.message : next[key];
