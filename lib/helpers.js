...
export function normalizeString(message: Message) {
  ...
  } else if (error.name && error.operator && error.actual && error.expected) {
    content = `${error.name}: ${JSON.stringify(error.actual)} ${error.operator || ''} ${JSON.stringify(error.expected)}`;
  }
