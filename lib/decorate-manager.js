...
export function setInlineMessages(editor: TextEditor, messages: Array<Message>) {
  if (!editor) {
    return;
  }
  const currentFilePath = editor.getPath();
  messages.forEach((message) => {
    if (currentFilePath !== message.filePath) {
      return;
    }
...
export function clearDecoratedGutter(editor: ?TextEditor, gutter: ?Gutter) {
  if (!editor || !gutter) {
    return;
  }

  editor.getDecorations({ gutterName: 'tester', type: 'gutter' }).forEach((decoration) => {
    decoration.destroy();
  });
}
