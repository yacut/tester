  ...
  */
  ...
  constructor(state/* : State*/) {
    ...
    this.registryTesters.onShouldTriggerTester(({ shouldTriggerTester }) => {
      ...
      if (shouldTriggerTester) {
        $(self.statusBarTile.getItem()).show();
      } else {
        $(self.statusBarTile.getItem()).hide();
      }
    });
  ...
  decorateGutter(editor/* :TextEditor*/, gutter/* :Gutter*/, messages/* : Array<Object>*/) {
    ...
    messages.forEach((message) => {
      const tooltipDuration = $(document.createElement('span')).text(`${message.duration}ms`).addClass('inline-block highlight-info');
      const tooltipTesterName = $(document.createElement('span')).text('Tester').addClass('inline-block highlight');
      const tooltipTesterState = $(document.createElement('span')).append(message.state).addClass('inline-block');
      ...
      const tooltipTitle = $(document.createElement('span')).append(tooltipTesterName, tooltipTesterState, tooltipDuration).addClass('inline-block');
      const tooltipError = $(document.createElement('div')).addClass('example-code show-example-html');
