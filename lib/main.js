...
import { union } from 'underscore-plus';
  ...
  constructor(state : State) {
    ...
    this.subscriptions.add(atom.workspace.addOpener((uri) => {
      if (uri === 'atom://tester-result-view') {
        ...
        self.resultView.onTestButtonClick(() => {
          self.registryTesters.test(null, true);
        });
    ...
    this.commands.onShouldTest(async () => {
    ...
    this.registryTesters.onDidUpdateMessages(async ({ editor, messages, output }) => {
      ...
      this.messages = union(this.messages, messages);
      this.output = output;
      await self.updateStatusBar();
      if (this.panel) {
