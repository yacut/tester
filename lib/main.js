  ...
  */
  ...
  constructor(state/* : State*/) {
    ...
    this.registryTesters.onDidUpdateMessages(({ editor, messages, output }) => {
      ...
      // TODO fix cleanup per TextEditor
      this.clearInlineMessages();
      // TODO add setting for inlineMessage on/off
      this.setInlineMessages(editor, messages);
  ...
  createStatusBar(statusBar/* : Object*/) {
    ...
    statusBarTile.setResults = function setResults(editor, results) {
      ...
      if (results && editor !== atom.workspace.getActiveTextEditor()) {
        ...
        const resultMessage = `${failedTests} failed, ${skippedTests} skipped, ${passedTests} passed`;
        atom.notifications.addInfo(`Tester: Results for ${editor.getTitle()}<br/>${resultMessage}`);
      }
      if (!results || editor !== atom.workspace.getActiveTextEditor()) {
        this.resetResults();
      } else {
  ...
  setInlineMessages(editor/* : TextEditor*/, messages/* : Array<Object>*/) {
    _.forEach(messages, (message) => {
      ...
      .text(`${message.error.name}: ${message.error.message}`);
