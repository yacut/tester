...
class Tester {
  ...
  constructor(initialState) {
    ...
    this.getStore = () => {
      if (self.store == null) {
        ...
        self.store.subscribe(async () => {
          console.log('store updated', self.store.getState());
          const resultState = self.getStore().getState().results;
          if (self.statusBarTile) {
            await self.statusBarTile.update(resultState);
          }
          if (this.resultView) {
            await this.resultView.update(resultState);
          }
          if (this.panel) {
            await this.panel.update(resultState);
          }
          if (resultState.editor) {
            const gutter = self.registryEditors.handleGutter(resultState.editor);
            if (!gutter) {
              return;
            }
            clearInlineMessages(resultState.editor);
            decorateGutter(resultState.editor, gutter, resultState.messages);
            if (self.showInlineError) {
              setInlineMessages(resultState.editor, resultState.messages, self.ansiToHtml, self.inlineErrorPosition);
            }
          }
        });
      }
    ...
    this.deserializeConsoleOutput = function deserializeConsoleOutput(serializedState :any) {
      console.log('serializedState', serializedState);
      self.panel = new ConsoleOutputView({
        state: self.getStore().getState().results,
    ...
    this.deserializeResultView = function deserializeResultView(serializedState :any) {
      console.log('serializedState', serializedState);
    ...
    this.registryTesters.onDidBeginTesting(async ({ editorTester }) => {
      ...
      self.getStore().dispatch(actions.increaseCounterAction());
    ...
    this.registryTesters.onDidFinishTesting(async () => {
      self.getStore().dispatch(actions.decreaseCounterAction());
      // self.runningTestersCount -= 1;
      // if (self.resultView) {
      //   await self.resultView.update({ runningTestersCount: self.runningTestersCount });
      // }
  ...
  createStatusBar(statusBar : Object) {
    ...
    this.statusBarTile = new StatusBarTile({ state: this.getStore().getState().results, onclick });
...
export default {
  ...
  serialize() {
    ...
    state.results.counter = 0;
    delete state.results.editor;
    return state;
  },
