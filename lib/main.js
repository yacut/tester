  ...
  */
  ...
  constructor(state/* : State*/) {
    ...
    this.registryTesters.onShouldTriggerTester(({ shouldTriggerTester }) => {
      console.log('onShouldTriggerTester', { shouldTriggerTester, TILE: self.statusBarTile, item: atom.views.getView(self.statusBarTile.getItem()) });
      if (shouldTriggerTester) {
        atom.views.getView(self.statusBarTile.getItem()).style.display = 'inline-block';
      } else {
        atom.views.getView(self.statusBarTile.getItem()).style.display = 'none';
      }
    });
  ...
  createStatusBar(statusBar/* : Object*/) {
    ...
    statusBarTile.testResults = function (results) {
      ...
      if (!results) {
        ...
        this.status.passed.className = `${baseClasses} tester-status-passed`;
        this.status.failed.className = `${baseClasses} tester-status-failed`;
        this.status.skipped.className = `${baseClasses} tester-status-skipped`;
      } else {
        const passedTests = results.filter(result => result.state === 'passed').length;
        this.status.passed.textContent = passedTests;
        this.status.passed.className = `${baseClasses} tester-status-passed ${passedTests > 0 ? 'highlight-success' : ''}`;

        const failedTests = results.filter(result => result.state === 'failed').length;
        this.status.failed.textContent = failedTests;
        this.status.failed.className = `${baseClasses} tester-status-failed ${failedTests > 0 ? 'highlight-error' : ''}`;

        const skippedTests = results.filter(result => result.state === 'skipped').length;
        this.status.skipped.textContent = skippedTests;
        this.status.skipped.className = `${baseClasses} tester-status-skipped ${skippedTests > 0 ? 'highlight-warning' : ''}`;
      }
    };
