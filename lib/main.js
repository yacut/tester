      ...
      if (self.store == null) {
        ...
        self.store.subscribe(async () => {
          ...
          if (self.statusBarTile) {
            await self.statusBarTile.update(self.getStore().getState().results);
          }
          if (this.resultView) {
            await this.resultView.update(self.getStore().getState().results);
          }
          if (this.panel) {
            await this.panel.update(self.getStore().getState().results);
          }
        });
      }
      return self.store;
...
export default {
  ...
  serialize() {
    ...
    return this.instance.getStore().getState();
  },
